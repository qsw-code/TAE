...
from cnn_tae import CNNTAE
from attentionxml import AttentionXML
from xmlcnn import XMLCNN
from lstm_att import LstmATT
from yy_lstm_tae import LstmTAE
from bertxml_tae import BertXMLTAE
from bertxmltc import BertXMLTC
from bertxml import BertXML
from cnntc import CNNTC
from lstmtc import LstmTC
from van_tae import VANTAE
model_dict = {
...
def main(data_cnf, model_cnf, mode):
    ...
    if mode is None or mode == 'train':
        ...
        train_loader = DataLoader(MultiLabelDataset(train_x, train_y),
                                  model_cnf['train']['batch_size'], shuffle=True, num_workers=5)
        valid_loader = DataLoader(MultiLabelDataset(valid_x, valid_y, training=True),
                                  model_cnf['valid']['batch_size'], num_workers=5)
    ...
    if mode is None or mode == 'eval':
        ...
        test_loader = DataLoader(MultiLabelDataset(test_x), model_cnf['predict']['batch_size'], num_workers=5)
        if 'gpipe' not in model_cnf:
            if model is None:
                model = Model(network=model_dict[model_name], labels_num=labels_num, model_path=model_path, emb_init=emb_init,
                              **data_cnf['model'], **model_cnf['model'])
