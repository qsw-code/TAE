...
from evaluation import get_p_5, get_n_5
...
#                    labels = np.concatenate([self.predict_step(valid_x, k)[1] for valid_x in valid_loader])
                    ...
                    if verbose:
                        ...
                        (epoch_idx, i * train_loader.batch_size, print_loss / step, valid_loss, np.round(p5, 5), np.round(n5, 5), e)
                        logger.info(log_msg)
                        print_loss = 0.0
    ...
    def clip_gradient(self):
        if self.gradient_clip_value is not None:
            ...
            if total_norm > max_norm * self.gradient_clip_value:
                logger.warn(F'Clipping gradients with total norm {np.round(total_norm.cpu(), 5)} '
                            F'and max norm {np.round(max_norm.cpu(), 5)}')
