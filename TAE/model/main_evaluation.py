...
from evaluation import get_p_1, get_p_3, get_p_5, get_n_1, get_n_3, get_n_5, get_inv_propensity
from evaluation import get_psp_1, get_psp_3, get_psp_5, get_psndcg_1, get_psndcg_3, get_psndcg_5
...
@click.option('--train-labels', typelick.Path(exists=True), default=None, help='Path of labels for training set.')
...
def main(results, targets, train_labels, a, b):
    res, target = np.load(results, allow_pickle=True), np.load(targets, allow_pickle=True)


    mlb = MultiLabelBinarizer(sparse_output=True)
    targets = mlb.fit_transform(target)
